...
export async function ViewedRecently({ product }: IViewedRecentlyProps) {
  ...
  const [viewedRecently, settingsDocumentSnapshot] = await Promise.all([
  ...
  viewedRecentlyEnriched = viewedRecentlyEnriched.sort(
    (a, b) => b.time - a.time
  );

  if (clientId && product && !viewedRecently[product?.id]) {
    const newViewRecently: IViewedRecentlyModel<IProductSerialized | string>[] =
      [...viewedRecentlyEnriched];
    ...
    const newViewedRecentlyObj: Record<
      string,
      IViewedRecentlyModel<DocumentReference>
    > = {};
          ...
          {viewedRecentlyEnriched.map((item) => (
            ...
            >
              <Image
                width={55}
                height={55}
                src={item.productRef?.imageUrls[0]}
                alt={item.productRef?.title}
              />
              <div className="ml-2">
                <div className="viewed-recently__card-title text-base">
                  {item.productRef?.title}
                </div>
                <div className="font-light">
                  {currency(item.productRef?.price).toString()}{' '}
                  {config.currency}
                </div>
              </div>
            </Link>
