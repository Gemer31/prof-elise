...
export function Header() {
  ...
  const navLinkClass: string = useMemo(
    () =>
      convertToClass([
        'flex',
        'items-center',
  ...
  const navSidebarLinkClass: string = useMemo(
    () =>
      convertToClass([
        'flex',
        'items-center',
  ...
  const getNavigationLinkClass = (path: string) =>
    navLinkClass + (path === pathname ? ' bg-white' : '');
  const getNavigationSidebarLinkClass = (path: string) =>
    navSidebarLinkClass + (path === pathname ? ' text-pink-500' : '');
  ...
  return (
    <header className={hostClass}>
      <CircleButton
        href="#page"
        styleClass={
          'size-10 shadow-md fixed max-w-fit bottom-6 left-6 duration-500 scale-0 ' +
