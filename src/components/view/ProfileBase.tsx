  ...
  return (
    <>
      ...
      <ContentContainer styleClass="flex flex-col items-center px-2">
        ...
        <h1 className="w-full text-2xl font-medium py-2">
          {TRANSLATES[LOCALE].privateAccount}
        </h1>
        <section className="w-full flex justify-between">
          <div className="flex gap-x-4">
            {PROFILE_TABS.map((item) =>
              item.href !== RouterPath.EDITOR ||
              (item.href === RouterPath.EDITOR &&
                ...
                <Button
                  ...
                  styleClass={
                    'flex px-4 py-2 ' +
                    (activeRoute === item.id ? 'pointer-events-none' : '')
                  }
                  href={item.href}
                >
                  {item.title}
        ...
        <article className="w-full my-4">{children}</article>
      </ContentContainer>
    </>
  );
}
