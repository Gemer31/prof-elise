  ...
  return (
    <div className={hostClass}>
      <div className="grid grid-cols-5">
        <div className="flex items-center font-medium text-lg z-10">
          № {data.number}
        </div>
        ...
        <div className="flex items-center">
          <span className={ORDER_STATUS_CLASSES.get(data.status)}>
            {TRANSLATES[LOCALE][data.status]}
          </span>
        </div>
        <div className="flex items-center">{data.comment || '—'}</div>
        <div className="flex justify-end items-center font-medium text-lg">
          {data.totalPrice}
        </div>
      </div>
      ...
      <div className="order-card-details w-full rounded-md p-2 mt-4 bg-white">
        {Object.values(data.products).map((item) => (
          <div key={item.id} className="grid-cols-3 grid">
            <Link
              className="flex hover:underline items-center flex-grow z-10 shrink"
