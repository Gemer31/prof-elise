...
export function OrderEditorForm({ refreshCallback }: IOrderEditorFormProps) {
  ...
  const statusChanged = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      const res = await setDoc(doc(db, FirestoreCollections.ORDERS, order.id), {
        ...order,
        status: newStatus,
      });
      ...
      dispatch(
        setNotificationMessage(TRANSLATES[LOCALE].orderUpdateSuccessfully)
      );
    } catch {
  ...
  return (
    <div>
      <SearchInput
        placeholder={TRANSLATES[LOCALE].enterOrderNumber}
      ...
      {searchIsLoading ? (
        ...
        <div className="text-center p-4">
          {TRANSLATES[LOCALE].enterOrderNumber}
        </div>
      )}
    </div>
  );
