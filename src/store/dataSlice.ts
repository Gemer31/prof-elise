...
export const dataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(updateCart.settled, (state: IDataSlice, action) => {
      const newCart = action.payload as Record<
        string,
        ICartProductModel<string>
      >;
      state.cartTotal = 0;
      newCart &&
    ...
    builder.addMatcher(
      updateFavourites.settled,
      (state: IDataSlice, action) => {
        state.favourites = action.payload as Record<string, string>;
      }
    );
    builder.addMatcher(
      updateViewedRecently.settled,
      (state: IDataSlice, action) => {
        state.viewedRecently = action.payload as Record<
          string,
          IViewedRecentlyModel<string>
        >;
      }
    );
...
export const { setPopupData, setNotificationMessage } = dataSlice.actions;
