...
export default async function CartPage() {
  const [cart, settingsQuerySnapshot] = await Promise.all([
    getClientData<Record<string, ICartProductModel>>(FirestoreCollections.CART, cookies()),
    getDocs(collection(db, FirestoreCollections.SETTINGS))
  ]);
  const config: IConfig = settingsQuerySnapshot.docs[0].data() as IConfig;
  const enrichedCart: Record<string, ICartProductModel<IProductSerialized>> = await getEnrichedCart(SerializationUtil.getSerializedCart(cart));
