...
export default async function ProductDetailsPage({
  ...
  const product: IProductSerialized = SerializationUtil.getSerializedProduct(
    productDocumentSnapshot.data() as IProduct
  );
  if (!product) {
    notFound();
  }

  const productCategory: ICategory = categories.find((item) =>
    product.categoryRef.includes(item.id)
  );
  if (!productCategory) {
    notFound();
  }
        ...
        <div className="w-full flex justify-between mb-4 flex-col-reverse md:flex-row ">
          <div className="w-full md:w-4/12 mr-4">
            <div className="sticky top-20">
              <Catalog
                pageLimit={pageLimit}
                currentCategoryId={product.categoryRef}
          ...
          <article className="w-full flex justify-between">
            <div className="w-full bg-slate-100 rounded-md p-4">
                ...
                <div className="flex">
                  <div className="relative rounded-md">
                      ...
                      {product.labels?.map((item, index) => {
                        return (
                          <div
                            key={index}
                            className={
                              'px-2 py-1 text-white rounded-md text-xs pointer-events-none ' +
                              COLOR_OPTION_VALUES.get(item.color)
                            }
                          >
                            {item.text}
                          </div>
                    ...
                    <EntityFavouriteButton
                      className="absolute scale-100 top-4 right-2"
              ...
              <section
                className="mt-4 ql-editor readonly-ql-editor no-paddings whitespace-pre-line"
